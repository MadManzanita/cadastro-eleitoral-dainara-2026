...
export default function PasswordRecovery({ role, initialCpf = "", leadershipId, onBack }) {
  ...
  async function send(event) {
    ...
    try {
      ...
      setChallengeId(data.challengeId); setCode(""); setSeconds(data.retryAfter); setMessage(data.message);
  ...
  async function reset(event) {
    ...
    setBusy(true); setMessage("");
    try {
      await call({ action: "reset", challengeId, code, password });
  ...
  return <main className="shell"><section className="card auth-card">
    ...
    {done ? <><p>Sua nova senha foi salva. Entre com seu CPF e a nova senha.</p><button className="primary" onClick={onBack}>Voltar para entrar</button></> : <>
      <p>{role === "leader" ? "Liderança" : "Ativista"}: receba um código por SMS no celular do seu cadastro.</p>
      {!challengeId ? <form onSubmit={send}>
        ...
        <form onSubmit={reset}>
          ...
          <label className="field"><span>Nova senha — 8 números</span><input required type="password" inputMode="numeric" autoComplete="new-password" pattern="[0-9]{8}" maxLength={8} value={password} onChange={(event) => setPassword(digits(event.target.value).slice(0, 8))} disabled={busy}/></label>
        ...
        <button type="button" className="link-button" disabled={busy} onClick={() => { setChallengeId(""); setCode(""); setPassword(""); setConfirmation(""); setMessage(""); }}>Corrigir CPF</button>
      </>}
