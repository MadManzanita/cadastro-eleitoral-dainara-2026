...
function Field({ f, setF, n, label, type = "text", required = false, placeholder }) {
  ...
  const guidance = {
    ...
    phone: "DDD e número do telefone",
    address: "Rua, número e complemento",
    mother: "Digite o nome completo da mãe",
    email: "exemplo@email.com",
    cep: "CEP do endereço",
    title: "12 números do título",
    zone: "Até 3 números",
    section: "Até 4 números",
  }[n];
  return (
    <label className="field">
      ...
      <input type={type} required={required} value={numeric ? maskField(n, f[n]) : f[n] || ""} placeholder={placeholder || guidance} maxLength={max} inputMode={numeric ? "numeric" : undefined} pattern={electoral ? "[0-9]*" : undefined} autoCapitalize={preserveCase ? "none" : undefined} autoCorrect={preserveCase ? "off" : undefined} spellCheck={preserveCase ? false : undefined} style={preserveCase ? { textTransform: "none" } : undefined} onChange={(e) => change(e.target.value)} />
    </label>
  );
}
...
function PixField({ f, setF }) {
  ...
  return (
    <label className="field">
      <span>Chave Pix</span>
      <input className="case-preserving-input" type="text" name="pix" value={String(f.pix || "")} placeholder="CPF, CNPJ, celular, e-mail ou chave aleatória" autoCapitalize="none" autoCorrect="off" autoComplete="off" spellCheck={false} onInput={(e) => updatePix(e.currentTarget.value)} onChange={(e) => updatePix(e.currentTarget.value)} />
    </label>
  );
}
...
function HolderNameField({ f }) {
  return (
    <label className="field">
      ...
      <input value={f.name || ""} placeholder="Preenchido automaticamente pelo nome completo" disabled aria-disabled="true" title="Preenchido automaticamente com o nome completo" />
    </label>
  );
}
      ...
      <div className="form-grid three">
        <label className="field">
          ...
          >
            ...
            {AMAZONAS_MUNICIPALITIES.map((municipality) => (
        ...
        <label className="field">
          <span>Bairro / localidade *</span>
          <select
            required
            disabled={!inferredMunicipality}
            ...
            onChange={(e) =>
              setF((x) => ({
                ...
                manausZone: inferredMunicipality === "Manaus" ? getManausZone(e.target.value) : "",
              }))
            }
          >
            <option value="">{inferredMunicipality ? "Selecione o bairro/localidade" : "Selecione primeiro o município"}</option>
        ...
        <label className="field">
          <span>Zona de Manaus</span>
          <input readOnly value={manausZone} placeholder={inferredMunicipality === "Manaus" ? "Detectada automaticamente" : "Somente para Manaus"} />
        </label>
      </div>
    </>
  );
  ...
  return (
    <label className="field">
      ...
      <select value={selected} onChange={(e) => choose(e.target.value)}>
        <option value="">Selecione o banco ou instituição Pix</option>
        {PIX_BANKS.map((bank) => (
          <option key={bank} value={bank}>
            {bank}
          </option>
        ))}
        <option value="__other__">Outros</option>
      </select>
      {other && (
        <input
          autoFocus
  ...
  return (
    <main className="shell">
      <section className="card wide">
        ...
        <h2>{edit ? `Editar ${kind}` : `Cadastro de ${kind}`}</h2>
        <p>Preencha os dados e salve o cadastro.</p>
        <form onSubmit={save}>
          <h3>Dados pessoais</h3>
          <div className="form-grid">
            <Field f={f} setF={setF} n="name" label="Nome completo" required />
            <Field f={f} setF={setF} n="birth" label="Data de nascimento" type="date" />
            <Field f={f} setF={setF} n="cpf" label="CPF" required placeholder="000.000.000-00" />
            <Field f={f} setF={setF} n="phone" label="Celular / telefone" placeholder="(00) 00000-0000" />
            <Field f={f} setF={setF} n="address" label="Endereço" />
            <Field f={f} setF={setF} n="mother" label="Nome da mãe" />
            <Field f={f} setF={setF} n="email" label="E-mail" type="email" />
            <Field f={f} setF={setF} n="cep" label="CEP" placeholder="00000-000" />
          </div>
          ...
          <div className="form-grid three">
            <Field f={f} setF={setF} n="title" label="Título de eleitor" required placeholder="000000000000" />
            <Field f={f} setF={setF} n="zone" label="Zona" />
            <Field f={f} setF={setF} n="section" label="Seção" />
          </div>
          <div className="validation-row">
            <button
              ...
              onClick={() =>
                setF((x) => ({
                  ...x,
                  _validation: `${cpfOK(x.cpf) ? "✓ CPF válido" : "✗ CPF inválido"} • ${titleOK(x.title) ? "✓ Título válido" : "✗ Título inválido"}`,
                }))
              }
            >
              Validar CPF e título
            </button>
            <a className="tse" href={TSE} target="_blank" rel="noreferrer">
              Consultar situação no TSE ↗
            </a>
          </div>
          <h3>Dados de pagamento</h3>
          <div className="form-grid three">
            <PixField f={f} setF={setF} />
            <HolderNameField f={f} />
            <BankField f={f} setF={setF} />
          </div>
          {f._validation && <div className="result">{f._validation}</div>}
          {msg && <div className="result">{msg}</div>}
          <button className="primary submit">{edit ? "Salvar alterações" : `Cadastrar ${kind}`}</button>
        </form>
      </section>
    </main>
  );
  ...
  return (
    <main className="shell">
      <section className="card auth-card">
        ...
        <h2>{release ? (admin ? "Se tornar administrador" : "Se torne liderança") : admin ? "Acesso administrativo" : "Já sou liderança"}</h2>
        ...
        {release ? (
          <label className="field">
            ...
            <input autoFocus inputMode="numeric" maxLength={6} value={code} placeholder="000000" onChange={(e) => setCode(digits(e.target.value).slice(0, 6))} onKeyDown={(e) => e.key === "Enter" && onEnter()} />
          </label>
        ) : (
          <>
            <label className="field">
              <span>CPF</span>
              <input autoFocus inputMode="numeric" maxLength={14} value={cpfBR(cpf)} placeholder="000.000.000-00" onChange={(e) => setCpf(digits(e.target.value).slice(0, 11))} onKeyDown={(e) => e.key === "Enter" && onEnter()} />
            </label>
            <label className="field">
              <span>Senha</span>
              <input type="password" minLength={8} value={password} placeholder={admin ? "Senha de administrador" : "Senha de acesso"} onChange={(e) => setPassword(admin ? e.target.value : e.target.value.replace(/\D/g, "").slice(0, 8))} onKeyDown={(e) => e.key === "Enter" && onEnter()} />
            </label>
          </>
        )}
        {msg && <div className="result">{msg}</div>}
  ...
  return (
    <main className="shell">
      <section className="card auth-card">
        ...
        <p>Crie suas credenciais de administrador. Depois do cadastro, o acesso será feito com CPF e senha.</p>
        <form onSubmit={save}>
          <label className="field">
          ...
          <label className="field">
            ...
            <input
              ...
              onChange={(e) => {
                e.currentTarget.value = cpfBR(e.currentTarget.value);
              }}
            />
          </label>
          <label className="field">
          ...
          <label className="field">
            <span>Crie uma senha de administrador *</span>
            <input name="password" type="password" required minLength={8} placeholder="Mínimo de 8 caracteres" />
          </label>
          {msg && <div className="result">{msg}</div>}
          <button className="primary">Concluir cadastro</button>
        </form>
      </section>
    </main>
  );
...
function Credential({ person, password, onAccess, onBack }) {
    ...
    return (
      <main className="shell">
        <section className="card auth-card">
          ...
          <p>O cadastro foi salvo, mas a tela de credenciais precisa ser recuperada.</p>
  ...
  return (
    <main className="shell">
      <section className="card auth-card credential-card">
        <div className="credential-success">✓ CADASTRO CONCLUÍDO</div>
        <h2>Senha da liderança gerada</h2>
        <p>{person.name}, guarde esta credencial. Ela será usada com seu CPF para entrar na Área da Liderança.</p>
        <div className="credential-box featured">
          <div className="credential-item">
            <span>CPF</span>
            <b>{cpfBR(person.cpf)}</b>
          </div>
          <div className="credential-divider" />
          <div className="credential-item">
            <span>SENHA DE ACESSO</span>
            <strong>{shownPassword}</strong>
          </div>
          <button className="copy-password" onClick={copy}>
            Copiar senha
          </button>
        </div>
        <div className="credential-warning">
          <b>Importante:</b> esta senha é necessária para acessar a Área da Liderança. Anote ou copie antes de continuar.
        </div>
        <button className="primary" onClick={onAccess}>
          Ir para acesso da liderança
        </button>
...
function AssessorForm({ form, setForm, save, cancel, editing }) {
  return (
    <div className="assessor-editor">
      <div className="editor-title">
        <div>
          <h3>{editing ? "Editar contato" : "Novo contato da assessoria"}</h3>
          <p>Este contato ficará disponível automaticamente para todas as lideranças.</p>
        </div>
        <button className="back small-back" onClick={cancel}>
          Cancelar
        </button>
      </div>
      <form onSubmit={save}>
        <div className="form-grid">
          ...
          <label className="field">
            <span>Função / descrição</span>
            <input value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} placeholder="Ex.: Assessoria, coordenação, jurídico..." />
          </label>
          <label className="field">
            <span>Telefone</span>
            <input value={form.phone} onChange={(e) => setForm({ ...form, phone: phoneBR(e.target.value) })} placeholder="(00) 00000-0000" />
          </label>
          <label className="field">
            <span>E-mail</span>
        ...
        <label className="field">
          ...
          <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} rows={3} placeholder="Informações adicionais para as lideranças..." />
        </label>
        <button className="primary">{editing ? "Salvar alterações" : "Adicionar contato"}</button>
      </form>
    </div>
  );
}
...
function AdminLeadershipCredential({ person, onReset }) {
  ...
  return (
    <div className="panel credential-admin">
      <h3>Credencial de acesso</h3>
      <p>Por segurança, a senha atual não pode ser exibida. Gere uma nova senha para repassar à liderança.</p>
      ...
      {newPassword && (
        <div className="credential-box">
          ...
          <p>Copie e entregue esta senha agora. A senha anterior foi invalidada.</p>
          <button type="button" className="copy-password" onClick={copy}>
            Copiar nova senha
          </button>
        </div>
      )}
...
function AdminActivistCredential({ person, onReset }) {
  const [newPassword, setNewPassword] = useState("");
  const copy = async () => {
    try {
      await navigator.clipboard.writeText(newPassword);
  ...
  const reset = async () => {
    const question = person.trustCredentialed ? "Gerar uma nova senha? A senha anterior deixará de funcionar." : "Gerar a primeira senha deste ativista?";
  ...
  return (
    <div className="panel credential-admin">
      <h3>Credencial da Rede de confiança</h3>
      <p>
        Status: <span className={`credential-status ${person.trustCredentialed ? "ready" : "pending"}`}>{person.trustCredentialed ? "Credenciado" : "Pendente"}</span>
      </p>
      <p>{person.trustCredentialed ? "O ativista já possui senha. Por segurança, ela não pode ser exibida." : "A senha será criada automaticamente no primeiro acesso. O administrador também pode gerá-la agora."}</p>
      <button type="button" className="outline" onClick={reset}>
        {person.trustCredentialed ? "Trocar senha" : "Gerar senha"}
      </button>
      {newPassword && (
        <div className="credential-box">
          <div className="credential-item">
          ...
          <p>Copie e entregue esta senha agora. Ela não será exibida novamente.</p>
          <button type="button" className="copy-password" onClick={copy}>
            Copiar nova senha
          </button>
        </div>
    ...
    return (
      <main className="shell">
        <section className="card">
          ...
          <div className="empty">Cadastro não encontrado.</div>
        </section>
      </main>
    );
  return (
      ...
      <section className="card wide">
        ...
        <div className="detail-grid">
          <div className="panel">
            ...
            <p>
              <b>Nascimento:</b> {dateBR(person.birth) || "—"}
            </p>
            <p>
              <b>CPF:</b> {cpfBR(person.cpf) || "—"}
            </p>
            <p>
              <b>Telefone:</b> {phoneBR(person.phone) || "—"}
            </p>
            <p>
              <b>E-mail:</b> {person.email || "—"}
            </p>
            <p>
              <b>Nome da mãe:</b> {person.mother || "—"}
            </p>
            <p>
              <b>Endereço:</b> {person.address || "—"}
            </p>
            <p>
              <b>Bairro:</b> {person.neighborhood || "—"}
            </p>
            <p>
              <b>CEP:</b> {cepBR(person.cep) || "—"}
            </p>
          </div>
          <div className="panel">
            <h3>Dados eleitorais</h3>
            <p>
              <b>Título:</b> {person.title || "—"}
            </p>
            <p>
              <b>Zona:</b> {person.zone || "—"}
            </p>
            <p>
              <b>Seção:</b> {person.section || "—"}
            </p>
            <h3>Dados de pagamento</h3>
            <p>
              <b>Pix:</b> {person.pix || "—"}
            ...
            <p>
              <b>Titular:</b> {person.pixname || "—"}
            </p>
            <p>
              <b>Banco:</b> {person.bank || "—"}
            </p>
          </div>
        </div>
        {credential}
  ...
  return (
    <section className="panel">
      ...
      <div style={{ position: "relative", maxWidth: 860, margin: "12px auto 0" }}>
        <canvas
          ref={canvasRef}
          width={MANAUS_MAP_WIDTH}
          height={MANAUS_MAP_HEIGHT}
          role="img"
          aria-label="Mapa interativo das zonas de Manaus"
          onMouseMove={(event) => setHovered(zoneAtPointer(event))}
          ...
          style={{
            ...
            borderRadius: 16,
            cursor: "default",
            filter: "drop-shadow(0 8px 14px rgba(157,91,116,.10))",
          }}
        />
        {active && (
          <div
          ...
          >
            {active.name}: {counts[active.name]} cadastro(s)
          </div>
        )}
      </div>
      <div
...
function TrustNetworkManager({ db, setDb, admin, remote }) {
  ...
  const list = families.filter((item) => {
    ...
    const origin = item.activistId ? "activist" : "leader";
    return text.includes(query.toLowerCase()) && (!activistFilter || item.activistId === activistFilter) && (!leaderFilter || item.leaderId === leaderFilter) && (!neighborhoodFilter || item.neighborhood === neighborhoodFilter) && (!originFilter || origin === originFilter);
  });
  const save = async (event) => {
    ...
    if (editingFamily.cpf && !cpfOK(editingFamily.cpf)) {
    ...
    try {
      const result = await remote("/api/families", {
        method: "POST",
        body: JSON.stringify({ action: "save", ...editingFamily }),
      });
      setDb((current) => ({
        ...current,
  ...
  const remove = async (item) => {
    ...
    try {
      await remote("/api/families", {
        method: "POST",
        body: JSON.stringify({ action: "delete", id: item.id }),
      });
      setDb((current) => ({
        ...current,
        families: current.families.filter((x) => x.id !== item.id),
      }));
  ...
  const showHistory = async (item) => {
    try {
      const result = await remote("/api/families", {
        method: "POST",
        body: JSON.stringify({ action: "history", id: item.id }),
      });
      setHistory(result.items || []);
      setNotice(`Histórico de ${item.name}`);
    } catch (error) {
      setNotice(error.message);
    }
  };
  const renderRow = (item) => {
    ...
    const leader = db.leaderships.find((x) => x.id === item.leaderId);
    return (
      <div className="assessor-admin-row" key={item.id}>
        <div className="assessor-info">
          <b>{item.name}</b>
          <small>
            {item.cpf ? cpfBR(item.cpf) : "CPF não informado"} • {item.neighborhood || "Sem bairro"}
          </small>
          <span>{item.activistId ? `Ativista responsável: ${activist?.name || "Não localizado"}` : `Cadastro direto da liderança: ${leader?.name || "Não localizada"}`}</span>
          <span>Liderança responsável: {leader?.name || "Não localizada"}</span>
        </div>
        <div className="assessor-actions">
          <button
            ...
            onClick={() => {
              setHistory([]);
              setEditingFamily({ ...EMPTY, ...item });
            }}
          >
            Editar
          </button>
          <button className="outline" onClick={() => showHistory(item)}>
            Histórico
          </button>
  ...
  const groups = [
    {
      key: "leader",
      title: "Cadastros feitos pela liderança",
      description: "Pessoas incluídas diretamente pela liderança responsável.",
      items: list.filter((item) => !item.activistId),
    },
    {
      key: "activist",
      title: "Cadastros feitos pelos ativistas",
      description: "Pessoas incluídas pelos ativistas através do link da liderança.",
      items: list.filter((item) => item.activistId),
    },
  ];
  return (
    <div className="panel">
      ...
      {editingFamily && (
        <form onSubmit={save} className="assessor-editor">
          ...
          <div className="form-grid">
            ...
            <Field f={editingFamily} setF={setEditingFamily} n="birth" label="Data de nascimento" type="date" />
            <Field f={editingFamily} setF={setEditingFamily} n="cpf" label="CPF" placeholder="000.000.000-00" />
            <Field f={editingFamily} setF={setEditingFamily} n="phone" label="Celular / telefone" />
            <Field f={editingFamily} setF={setEditingFamily} n="address" label="Endereço" required />
            <Field f={editingFamily} setF={setEditingFamily} n="mother" label="Nome da mãe" />
            <Field f={editingFamily} setF={setEditingFamily} n="email" label="E-mail" type="email" />
            <Field f={editingFamily} setF={setEditingFamily} n="cep" label="CEP" />
          </div>
          <TerritoryFields f={editingFamily} setF={setEditingFamily} />
          <h3>Dados eleitorais</h3>
          <div className="form-grid three">
            <Field f={editingFamily} setF={setEditingFamily} n="title" label="Título de eleitor" />
      ...
      {list.length ? (
        groups.map((group) => (
          <section className="trust-origin-group" key={group.key}>
            <div className="trust-origin-head">
              <div>
                <h3>{group.title}</h3>
            ...
            {group.items.length ? group.items.map(renderRow) : <div className="empty compact">Nenhum cadastro nesta categoria.</div>}
          </section>
        ))
      ) : (
        <div className="empty">Nenhum cadastro encontrado.</div>
      )}
    </div>
  );
}
...
function Side({ admin, view, setView, logout, newL, newA, exportExcel, exportCSV }) {
  const item = (name, label, icon) => (
    <button className={view === name ? "active" : ""} onClick={() => setView(name)}>
      {icon} {label}
    </button>
  );
  return (
    <aside className="sidebar">
      <div className="brand">
        ...
        <span>{admin ? "Coordenadora Dainara Torres" : "Área da liderança"}</span>
      </div>
      ...
      <nav>
        ...
        {admin ? (
          <>
            ...
            <button onClick={newL}>＋ Nova liderança</button>
          </>
        ) : (
          <>
            {item("activists", "Meus ativistas", "♧")}
            <button onClick={newA}>＋ Adicionar ativista</button>
            {item("trust-network", "Rede de confiança", "♡")}
            {item("daily-activities", "Registro de atividade", "▣")}
            {item("profile", "Meu cadastro", "👤")}
            {item("assessors", "Contatos da assessoria", "☎")}
          </>
        )}
      </nav>
      {admin && (
        <>
          <button className="export-side" onClick={exportExcel}>
            ⇩ Exportar Excel
          </button>
          <button className="export-side" onClick={exportCSV}>
            ⇩ Exportar CSV
          </button>
        </>
      )}
...
export default function Portal() {
  const [mode, setMode] = useState("home"),
    ...
    [db, setDb] = useState(fresh()),
    [role, setRole] = useState(null),
    [leaderId, setLeaderId] = useState(null),
    [view, setView] = useState("dashboard"),
    [search, setSearch] = useState(""),
    [detail, setDetail] = useState(null),
    [editing, setEditing] = useState(null),
    [returnTo, setReturnTo] = useState("home"),
    [generatedPassword, setGeneratedPassword] = useState(""),
    [generatedPerson, setGeneratedPerson] = useState(null),
    [assessorForm, setAssessorForm] = useState(EMPTY_ASSESSOR),
    [editingAssessor, setEditingAssessor] = useState(null),
    [assessorEditorOpen, setAssessorEditorOpen] = useState(false),
    [editingAdmin, setEditingAdmin] = useState(null),
    [adminForm, setAdminForm] = useState({ name: "", email: "" });
    ...
    try {
      ...
      if (raw) {
        ...
        setDb({
          ...fresh(),
          ...parsed,
          assessors: Array.isArray(parsed.assessors) ? parsed.assessors : [],
        });
      }
    } catch (e) {
      console.error(e);
  ...
  useEffect(() => {
    try {
      if (mode === "leader-credentials" && !generatedPerson) {
        const raw = sessionStorage.getItem(`${KEY}-new-leadership`);
        if (raw) {
          const saved = JSON.parse(raw);
          setGeneratedPerson(saved.person || null);
          setGeneratedPassword(String(saved.password || saved.person?.password || ""));
        }
      }
    } catch (e) {
      console.error(e);
  ...
  const leader = db.leaderships.find((x) => x.id === leaderId),
    mine = db.activists.filter((x) => x.leaderId === leaderId);
  ...
  const enterAdmin = async () => {
    ...
    try {
      const logged = await remote("/api/auth", {
        method: "POST",
        body: JSON.stringify({ action: "login", role: "admin", cpf, password }),
      });
      await loadRemote();
      setRole("admin");
      setLeaderId(null);
  ...
  const release = (admin) => {
    setF(EMPTY);
    setEditing(null);
    setReturnTo(admin ? "admin-area" : "home");
    go(admin ? "admin-reg" : "leader-reg");
  };
  ...
  const saveLeader = async (e) => {
    ...
    try {
      ...
      const result = await remote(isAdmin ? "/api/data" : "/api/auth", {
        method: "POST",
        body: JSON.stringify(
          isAdmin
            ? {
                action: "save-leadership",
                ...f,
                pixname: f.name,
                id: editing?.id,
              }
            : {
                action: "register-leadership",
                ...f,
                pixname: f.name,
                setupCode: code,
              },
      ...
      setDb((d) => ({
        ...d,
        leaderships: editing ? d.leaderships.map((a) => (a.id === x.id ? x : a)) : [...d.leaderships, x],
      }));
      setLeaderId(x.id);
      setEditing(null);
      setF(EMPTY);
      if (!editing) {
        setGeneratedPerson(x);
        setGeneratedPassword(result.temporaryPassword || "");
        go("leader-credentials");
      } else {
        setView("dashboard");
        go(returnTo);
      }
  ...
  const saveAct = async (e) => {
    ...
    try {
      const result = await remote("/api/data", {
        method: "POST",
        body: JSON.stringify({
          action: "save-activist",
          ...f,
          pixname: f.name,
          id: editing?.id,
          leaderId,
      ...
      setDb((d) => ({
        ...d,
        activists: editing ? d.activists.map((a) => (a.id === x.id ? x : a)) : [...d.activists, x],
      }));
      setEditing(null);
      setF(EMPTY);
      setView("activists");
  ...
  const startL = () => {
    setF(EMPTY);
    setEditing(null);
    setReturnTo(role === "admin" ? "admin-area" : "home");
    go("leader-form");
  };
  const startA = (lid = leaderId) => {
    setLeaderId(lid || null);
    setF(EMPTY);
    setEditing(null);
    setReturnTo(role === "admin" ? "admin-area" : "leader-area");
    go("activist-form");
  };
  const edit = (x, k) => {
    setF({ ...EMPTY, ...x });
    setEditing(x);
    setLeaderId(x.leaderId || leaderId);
    setReturnTo(role === "admin" ? "admin-area" : "leader-area");
    go(k === "leader" ? "leader-form" : "activist-form");
  };

  const startAssessor = () => {
    setEditingAssessor(null);
    setAssessorForm({ ...EMPTY_ASSESSOR });
    setMsg("");
    setAssessorEditorOpen(true);
    setView("assessors");
  };
  const editAssessor = (a) => {
    setEditingAssessor(a);
    setAssessorForm({ ...EMPTY_ASSESSOR, ...a });
    setAssessorEditorOpen(true);
    setMsg("");
  };
  const cancelAssessor = () => {
    setEditingAssessor(null);
    setAssessorForm(EMPTY_ASSESSOR);
    setAssessorEditorOpen(false);
  };
  const saveAssessor = async (e) => {
    ...
    try {
      ...
      setDb((d) => ({
        ...d,
        assessors: editingAssessor ? d.assessors.map((a) => (a.id === item.id ? item : a)) : [...d.assessors, item],
      }));
      setMsg(editingAssessor ? "Contato da assessoria atualizado." : "Contato da assessoria adicionado.");
      cancelAssessor();
    } catch (error) {
      setMsg(error.message);
    }
  ...
  const removeAssessor = async (id) => {
    ...
    try {
      await remote("/api/data", {
        method: "POST",
        body: JSON.stringify({ action: "delete-assessor", id }),
      });
      setDb((d) => ({
        ...d,
        assessors: d.assessors.filter((a) => a.id !== id),
      }));
  ...
  const archiveLeadership = async (item) => {
    ...
    try {
      const result = await remote("/api/data", {
        method: "POST",
        body: JSON.stringify({ action: "archive-leadership", id: item.id }),
      });
      setDb((current) => ({
        ...current,
      ...
      setSearch("");
      setView("archived");
      setMsg("Liderança arquivada. O acesso dela foi bloqueado.");
    } catch (error) {
      setMsg(error.message);
    }
  };
  const restoreLeadership = async (item) => {
    ...
    try {
      const result = await remote("/api/data", {
        method: "POST",
        body: JSON.stringify({ action: "restore-leadership", id: item.id }),
      });
      setDb((current) => ({
        ...current,
  ...
  const deleteLeadership = async (item) => {
    ...
    try {
      await remote("/api/data", {
        method: "POST",
        body: JSON.stringify({ action: "delete-leadership", id: item.id }),
      });
      setDb((current) => ({
        ...current,
        leaderships: current.leaderships.filter((leader) => leader.id !== item.id),
      ...
      setDetail(null);
      setView("archived");
      setMsg("Liderança e dados vinculados excluídos definitivamente.");
    } catch (error) {
      setMsg(error.message);
    }
  };
  ...
  const saveAdminProfile = async (e) => {
    ...
    try {
      ...
      setDb((current) => ({
        ...current,
        admins: current.admins.map((item) => (item.id === result.item.id ? result.item : item)),
      }));
      cancelAdminEdit();
      setMsg("Dados do administrador atualizados.");
    } catch (error) {
      setMsg(error.message);
    }
  };
  ...
  const exportExcel = async () => {
    try {
      ...
      const subtitle = {
        font: { name: "Aptos", sz: 10, italic: true, color: { rgb: "8A707A" } },
        fill: { fgColor: { rgb: PALE } },
        alignment: { vertical: "center" },
        border,
      };
      const section = {
        font: { name: "Aptos", sz: 12, bold: true, color: { rgb: TEXT } },
      ...
      const cell = {
        font: { name: "Aptos", sz: 10, color: { rgb: TEXT } },
      ...
      const metric = {
        font: { name: "Aptos", sz: 20, bold: true, color: { rgb: PINK } },
        fill: { fgColor: { rgb: PALE } },
        alignment: { horizontal: "center", vertical: "center" },
        border,
      };
      const metricLabel = {
        font: { name: "Aptos", sz: 10, bold: true, color: { rgb: TEXT } },
        fill: { fgColor: { rgb: LIGHT } },
        alignment: { horizontal: "center", vertical: "center" },
        border,
      };
      ...
      const nav = (ws, a, label, target) =>
        (ws[a] = {
          t: "s",
          v: label,
          s: link,
          l: { Target: `#'${target}'!A1`, Tooltip: `Ir para ${target}` },
        });
      const textCols = (ws, row, cols) =>
        cols.forEach((c) => {
          const a = XLSX.utils.encode_cell({ r: row, c });
          if (ws[a]) {
            ws[a].t = "s";
            ws[a].v = String(ws[a].v ?? "");
          }
        });
      ...
      panel["!cols"] = [{ wch: 34 }, { wch: 24 }, { wch: 24 }, { wch: 24 }];
      panel["A1"].s = title;
      panel["A2"].s = subtitle;
      styleRange(panel, "A4:B4", section);
      [5, 6, 7, 8].forEach((r) => {
        panel[`A${r}`].s = metricLabel;
        panel[`B${r}`].s = metric;
      });
      panel["A10"].s = section;
      nav(panel, "A11", "Lideranças →", "LIDERANÇAS");
      nav(panel, "A12", "Ativistas →", "ATIVISTAS");
      nav(panel, "A13", "Administradores →", "ADMINISTRADORES");
      nav(panel, "A14", "Assessoria →", "ASSESSORIA");
      panel["!merges"] = [
        { s: { r: 0, c: 0 }, e: { r: 0, c: 3 } },
        { s: { r: 1, c: 0 }, e: { r: 1, c: 3 } },
        { s: { r: 3, c: 0 }, e: { r: 3, c: 3 } },
      ...
      panel["!freeze"] = { xSplit: 0, ySplit: 4 };
      add(panel, "PAINEL");
      const lHeader = ["Nome completo", "Data de nascimento", "CPF", "Telefone", "Endereço", "Nome da mãe", "E-mail", "Bairro", "CEP", "Título de eleitor", "Zona", "Seção", "Chave Pix", "Titular Pix", "Banco", "Qtd. ativistas"];
      const lData = [["LIDERANÇAS"], ["Cadastro completo das lideranças vinculadas à coordenação."], [""], lHeader, ...db.leaderships.map((l) => [l.name, dateBR(l.birth), cpfBR(l.cpf), phoneBR(l.phone), l.address, l.mother, l.email, l.neighborhood, cepBR(l.cep), l.title, l.zone, l.section, l.pix, l.pixname, l.bank, db.activists.filter((a) => a.leaderId === l.id).length])];
      const ls = XLSX.utils.aoa_to_sheet(lData);
      ls["!cols"] = [30, 18, 18, 18, 32, 30, 30, 20, 14, 18, 10, 10, 28, 25, 20, 16].map((wch) => ({ wch }));
      ls["A1"].s = title;
      ls["A2"].s = subtitle;
      styleRange(ls, "A4:P4", header);
      for (let r = 4; r < lData.length; r++) {
        for (let c = 0; c < 16; c++) {
          const a = XLSX.utils.encode_cell({ r, c });
          if (ls[a]) ls[a].s = c === 0 ? link : cell;
        }
        textCols(ls, r, [1, 2, 3, 8, 9, 10, 11]);
      }
      ls["!freeze"] = { xSplit: 0, ySplit: 4 };
      ls["!autofilter"] = { ref: `A4:P${Math.max(4, lData.length)}` };
      nav(ls, "A2", "← Voltar ao painel", "PAINEL");
      add(ls, "LIDERANÇAS");
      const aHeader = ["Liderança", "Nome completo", "Data de nascimento", "CPF", "Telefone", "Endereço", "Nome da mãe", "E-mail", "Bairro", "CEP", "Título de eleitor", "Zona", "Seção", "Chave Pix", "Titular Pix", "Banco"];
      const aData = [["ATIVISTAS POR LIDERANÇA"], ["Equipes separadas visualmente para não misturar ativistas entre lideranças."], [""], aHeader];
      [...db.leaderships]
        .sort((a, b) => String(a.name).localeCompare(String(b.name), "pt-BR"))
        .forEach((l) => {
          aData.push([`LIDERANÇA: ${l.name}`]);
          const team = db.activists.filter((a) => a.leaderId === l.id).sort((a, b) => String(a.name).localeCompare(String(b.name), "pt-BR"));
          if (team.length) team.forEach((a) => aData.push([l.name, a.name, dateBR(a.birth), cpfBR(a.cpf), phoneBR(a.phone), a.address, a.mother, a.email, a.neighborhood, cepBR(a.cep), a.title, a.zone, a.section, a.pix, a.pixname, a.bank]));
          else aData.push([l.name, "Nenhum ativista vinculado"]);
        });
      const as = XLSX.utils.aoa_to_sheet(aData);
      as["!cols"] = [30, 30, 18, 18, 18, 32, 30, 30, 20, 14, 18, 10, 10, 28, 25, 20].map((wch) => ({ wch }));
      as["A1"].s = title;
      as["A2"].s = subtitle;
      styleRange(as, "A4:P4", header);
      as["!merges"] = [
        { s: { r: 0, c: 0 }, e: { r: 0, c: 15 } },
        { s: { r: 1, c: 0 }, e: { r: 1, c: 15 } },
      ];
      for (let r = 4; r < aData.length; r++) {
        const first = String(as[`A${r + 1}`]?.v || "");
        if (first.startsWith("LIDERANÇA:")) {
          styleRange(as, `A${r + 1}:P${r + 1}`, section);
          as["!merges"].push({ s: { r, c: 0 }, e: { r, c: 15 } });
        } else {
          for (let c = 0; c < 16; c++) {
            const a = XLSX.utils.encode_cell({ r, c });
            if (as[a]) as[a].s = cell;
          }
          textCols(as, r, [2, 3, 4, 9, 10, 11, 12]);
        }
      }
      as["!freeze"] = { xSplit: 0, ySplit: 4 };
      as["!autofilter"] = { ref: `A4:P${Math.max(4, aData.length)}` };
      nav(as, "A2", "← Voltar ao painel", "PAINEL");
      add(as, "ATIVISTAS");
      const adData = [["ADMINISTRADORES"], ["Administradores autorizados no sistema."], [""], ["Nome completo", "CPF", "E-mail", "Data do cadastro"], ...db.admins.map((a) => [a.name, cpfBR(a.cpf), a.email, new Date(a.created).toLocaleString("pt-BR")])];
      const ad = XLSX.utils.aoa_to_sheet(adData);
      ad["!cols"] = [32, 20, 34, 24].map((wch) => ({ wch }));
      ad["A1"].s = title;
      ad["A2"].s = subtitle;
      styleRange(ad, "A4:D4", header);
      for (let r = 4; r < adData.length; r++) {
        styleRange(ad, `A${r + 1}:D${r + 1}`, cell);
        textCols(ad, r, [1]);
      }
      ad["!freeze"] = { xSplit: 0, ySplit: 4 };
      ad["!autofilter"] = { ref: `A4:D${Math.max(4, adData.length)}` };
      nav(ad, "A2", "← Voltar ao painel", "PAINEL");
      add(ad, "ADMINISTRADORES");
      const scData = [["ASSESSORIA"], ["Contatos disponibilizados pela coordenação às lideranças."], [""], ["Nome", "Função / descrição", "Telefone", "E-mail", "Observações"], ...db.assessors.map((a) => [a.name, a.role, a.phone, a.email, a.notes])];
      const sc = XLSX.utils.aoa_to_sheet(scData);
      sc["!cols"] = [34, 30, 22, 36, 50].map((wch) => ({ wch }));
      sc["A1"].s = title;
      sc["A2"].s = subtitle;
      styleRange(sc, "A4:E4", header);
      for (let r = 4; r < scData.length; r++) styleRange(sc, `A${r + 1}:E${r + 1}`, cell);
      sc["!freeze"] = { xSplit: 0, ySplit: 4 };
      sc["!autofilter"] = { ref: `A4:E${Math.max(4, scData.length)}` };
      nav(sc, "A2", "← Voltar ao painel", "PAINEL");
      add(sc, "ASSESSORIA");
      XLSX.writeFile(wb, `cadastro-eleitoral-dainara-${new Date().toISOString().slice(0, 10)}.xlsx`, { bookType: "xlsx", compression: true });
      setMsg("Excel exportado com painel, navegação, filtros e equipes separadas.");
    } catch (e) {
      console.error(e);
      setMsg("Não foi possível gerar o Excel. Tente novamente.");
    }
  };

  const exportCSV = () => {
    const h = ["Liderança", "Nome completo", "Data de nascimento", "CPF", "Telefone", "Endereço", "Nome da mãe", "E-mail", "Bairro", "CEP", "Título de eleitor", "Zona", "Seção", "Chave Pix", "Titular Pix", "Banco"];
    const rows = [];
    db.leaderships.forEach((l) => db.activists.filter((a) => a.leaderId === l.id).forEach((a) => rows.push([l.name, a.name, dateBR(a.birth), cpfBR(a.cpf), phoneBR(a.phone), a.address, a.mother, a.email, a.neighborhood, cepBR(a.cep), a.title, a.zone, a.section, a.pix, a.pixname, a.bank])));
    const csv = [h, ...rows].map((r) => r.map((v) => `"${String(v ?? "").replace(/"/g, '""')}"`).join(";")).join("\r\n");
    const blob = new Blob(["\uFEFF" + csv], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `cadastro-eleitoral-dainara-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
    ...
    return (
      <main className="shell">
        <section className="hero">
          ...
          <p>Portal de acesso, cadastro e gestão de lideranças e ativistas.</p>
          <div className="actions">
            <button onClick={() => go("leader-access")}>Já sou liderança</button>
            <button onClick={() => go("leader-release")}>Se torne liderança</button>
            <button className="outline" onClick={() => go("admin-access")}>
              Acesso administrativo
            </button>
            <button className="outline" onClick={() => go("admin-release")}>
              Se tornar administrador
            </button>
          </div>
          <div className="home-note">Cadastros protegidos e armazenados no sistema.</div>
        </section>
      </main>
    );
  if (["leader-access", "admin-access", "leader-release", "admin-release"].includes(mode)) {
    const admin = mode.includes("admin"),
      rel = mode.includes("release");
    return <Access admin={admin} release={rel} cpf={cpf} setCpf={setCpf} password={password} setPassword={setPassword} code={code} setCode={setCode} msg={msg} onBack={() => go("home")} onEnter={() => (rel ? release(admin) : admin ? enterAdmin() : enterLeader())} onSwitch={() => go(rel ? (admin ? "admin-access" : "leader-access") : admin ? "admin-release" : "leader-release")} />;
  }
  ...
  if (mode === "leader-credentials")
    return (
      <Credential
        ...
        onAccess={() => {
          setRole("leader");
          setLeaderId(generatedPerson?.id || null);
          setView("dashboard");
          go("leader-area");
        }}
        onBack={() => go("home")}
  ...
  const admin = role === "admin",
    areaMode = admin ? "admin-area" : "leader-area";
  if (mode !== areaMode) {
    go(areaMode);
    return null;
  }
  ...
  if (view === "dashboard")
    content = admin ? (
      <>
        <div className="cards">
          ...
          <div className="metric">
            <div className="label">Ativistas</div>
            <div className="value">{db.activists.length}</div>
          </div>
          <div className="metric">
            <div className="label">Credenciados na rede</div>
            <div className="value">
              {credentialedActivists}/{db.activists.length}
            </div>
          </div>
          <div className="metric">
            <div className="label">Administradores</div>
            <div className="value">{db.admins.length}</div>
          </div>
          <div className="metric">
            <div className="label">Assessoria</div>
            <div className="value">{db.assessors.length}</div>
          </div>
        </div>
        <div className="quick-grid">
          <button className="quick" onClick={() => setView("leaderships")}>
            <b>♙ Lideranças</b>
            <span>Visualizar e gerenciar todas as lideranças.</span>
          </button>
          <button className="quick" onClick={() => setView("activists")}>
            <b>♧ Ativistas</b>
            <span>Consultar ativistas e suas lideranças responsáveis.</span>
          </button>
          <button className="quick" onClick={() => setView("assessors")}>
            <b>☎ Assessoria</b>
            <span>Adicionar e editar os contatos mostrados às lideranças.</span>
          </button>
          <button className="quick" onClick={() => go("admin-release")}>
            <b>＋ Novo administrador</b>
            <span>Liberar um novo cadastro com o código de teste.</span>
          </button>
        </div>
        <div className="panel">
          <div className="page-head">
            <div>
              <h2>Visão geral</h2>
              <p>Gerencie os cadastros e mantenha as equipes separadas por liderança.</p>
            </div>
            <button className="primary" onClick={startL}>
              ＋ Nova liderança
            </button>
          </div>
        </div>
      ...
      <>
        <div className="cards">
          ...
          <div className="metric">
            <div className="label">Meu cadastro</div>
        ...
        <div className="quick-grid">
          <button className="quick" onClick={() => setView("activists")}>
            <b>♧ Meus ativistas</b>
            <span>Veja somente os ativistas vinculados à sua liderança.</span>
          </button>
          <button className="quick" onClick={() => startA()}>
            <b>＋ Adicionar ativista</b>
            <span>Cadastre um novo ativista vinculado a você.</span>
          </button>
          <button className="quick" onClick={() => setView("profile")}>
            <b>👤 Meu cadastro</b>
            <span>Consulte e edite seus próprios dados.</span>
          </button>
          <button className="quick" onClick={() => setView("assessors")}>
            <b>☎ Assessoria</b>
            <span>Veja os contatos disponibilizados pela coordenação.</span>
          </button>
          <button
            className="quick"
            onClick={async () => {
              const link = window.location.origin + "/familia?lideranca=" + encodeURIComponent(leaderId || "");
              try {
                await navigator.clipboard.writeText(link);
                setMsg("Link da Rede de confiança copiado. Compartilhe-o com os ativistas da sua liderança.");
              } catch {
                setMsg("Não foi possível copiar o link automaticamente.");
              }
            }}
          >
            <b>♡ Copiar link da Rede de confiança</b>
            <span>Compartilhe este link com os ativistas vinculados à sua liderança.</span>
          </button>
        </div>
      </>
    );
  else if (view === "leaderships" && admin)
    content = (
      <div className="panel">
        <div className="page-head">
          <div>
            <h2>Lideranças</h2>
            <p>Todas as lideranças cadastradas pela coordenação.</p>
          </div>
          <button className="primary" onClick={startL}>
            ＋ Nova liderança
          </button>
        </div>
        <input className="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Pesquisar por nome, CPF ou título..." />
        {leaders.length ? (
          leaders.map((l) => (
            <div
              ...
              onClick={() => {
                setDetail(l.id);
                setView("leadership-detail");
              }}
            >
              <div>
                <b>{l.name}</b>
                <small>
                  {cpfBR(l.cpf)} • Título {l.title}
                </small>
              </div>
              <span>{db.activists.filter((a) => a.leaderId === l.id).length} ativista(s) →</span>
            </div>
          ))
        ) : (
          <div className="empty">Nenhuma liderança encontrada.</div>
        )}
      </div>
    );
  else if (view === "archived" && admin)
    content = (
      <div className="panel">
        <div className="page-head">
          <div>
            <h2>Arquivados</h2>
            <p>Lideranças com acesso bloqueado. Restaure o cadastro ou faça a exclusão definitiva.</p>
          </div>
        </div>
        <input className="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Pesquisar arquivados por nome, CPF ou título..." />
        {archivedLeaders.length ? (
          archivedLeaders.map((l) => (
            <div className="assessor-admin-row" key={l.id}>
              <div className="assessor-info">
                <b>{l.name}</b>
                <small>
                  {cpfBR(l.cpf)} • Título {l.title || "—"}
                </small>
                <span>Arquivado em {l.archivedAt ? new Date(l.archivedAt).toLocaleString("pt-BR") : "—"}</span>
                <span>{db.archivedActivists.filter((a) => a.leaderId === l.id).length} ativista(s) vinculado(s)</span>
              </div>
              <div className="assessor-actions">
                <button className="outline" onClick={() => restoreLeadership(l)}>
                  Restaurar
                </button>
                <button className="danger" onClick={() => deleteLeadership(l)}>
          ...
          <div className="empty">Nenhuma liderança arquivada.</div>
        )}
      </div>
    );
  else if (view === "activists") {
    const list = admin ? acts : mine.filter((a) => `${a.name} ${a.cpf} ${a.title}`.toLowerCase().includes(search.toLowerCase()));
    content = (
      <div className="panel">
        <div className="page-head">
          <div>
            <h2>{admin ? "Ativistas" : "Meus ativistas"}</h2>
            <p>{admin ? "A liderança aparece em cada linha para não misturar equipes." : `Liderança: ${leader?.name || "não cadastrada"}`}</p>
          </div>
          <button className="primary" onClick={() => startA()}>
            ＋ Adicionar ativista
          </button>
        </div>
        <input className="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Pesquisar por nome, CPF ou título..." />
        {list.length ? (
          list.map((a) => {
            const l = db.leaderships.find((x) => x.id === a.leaderId);
            return (
              <div
                className="list-item clickable activist-row"
                key={a.id}
                onClick={() => {
                  setDetail(a.id);
                  setView("activist-detail");
                }}
              >
                <div>
                  <b>{a.name}</b>
                  <small>
                    {cpfBR(a.cpf)} • Título {a.title}
                  </small>
                </div>
                <div className="activist-tags">
                  <span className="leader-tag">👤 {l?.name || "Sem liderança"}</span>
                  <span className={`credential-status ${a.trustCredentialed ? "ready" : "pending"}`}>{a.trustCredentialed ? "Credenciado" : "Pendente"}</span>
                </div>
              </div>
            );
          })
        ) : (
          <div className="empty">Nenhum ativista encontrado.</div>
        )}
      </div>
    );
    ...
    const l = db.leaderships.find((x) => x.id === detail),
      team = db.activists.filter((x) => x.leaderId === detail);
    content = (
      <Detail
        title="Liderança"
        person={l}
        onBack={() => setView("leaderships")}
        onEdit={() => edit(l, "leader")}
        actions={
          <>
            <button className="outline" onClick={() => archiveLeadership(l)}>
              Arquivar
            </button>
            <button className="danger" onClick={() => deleteLeadership(l)}>
              Excluir
            ...
            onReset={async () => {
              ...
              try {
                await remote("/api/auth", {
                  method: "POST",
                  body: JSON.stringify({
                    action: "reset-leadership-password",
                    leadershipId: l.id,
                    password,
                  }),
                });
                setMsg("Nova senha gerada. Copie-a e entregue à liderança.");
                return password;
              } catch (error) {
                setMsg(error.message);
                return "";
        ...
        team={
          <div className="panel team-panel">
            <div className="page-head">
              <div>
                <h3>Ativistas vinculados: {team.length}</h3>
                <p>Equipe delimitada para facilitar a gestão.</p>
              </div>
              <button className="primary" onClick={() => startA(l?.id)}>
                ＋ Adicionar ativista
              </button>
            </div>
            {team.length ? (
  ...
  } else if (view === "activist-detail") {
    const a = db.activists.find((x) => x.id === detail),
      l = db.leaderships.find((x) => x.id === a?.leaderId);
    content = (
      <Detail
        title="Cadastro do ativista"
        person={a}
        leader={l?.name}
        ...
        credential={
          admin ? (
            <AdminActivistCredential
              ...
              onReset={async () => {
                try {
                  const result = await remote("/api/family-access", {
                    method: "POST",
                    body: JSON.stringify({
                      action: "reset-activist-password",
                      activistId: a.id,
                    }),
                  });
                  setDb((current) => ({
                    ...current,
                    activists: current.activists.map((item) =>
                      item.id === a.id
                        ? {
                            ...item,
                            trustCredentialed: true,
                            trustCredentialedAt: result.activist.trustCredentialedAt,
                          }
                        : item,
                    ),
                  }));
                  setMsg("Nova senha gerada. Copie-a e entregue ao ativista.");
  ...
  else if (view === "assessors" && admin)
    content = (
      <div className="panel">
        <div className="page-head">
          <div>
            <h2>Contatos da assessoria</h2>
            <p>Cadastre aqui os contatos que aparecerão automaticamente na Área da Liderança.</p>
          </div>
          <button
            type="button"
            className="primary"
        ...
        {assessorEditorOpen && (
          <div className="assessor-modal-backdrop" role="dialog" aria-modal="true">
            <div className="assessor-modal">
              <AssessorForm form={assessorForm} setForm={setAssessorForm} save={saveAssessor} cancel={cancelAssessor} editing={editingAssessor} />
            </div>
          </div>
        )}{" "}
        {!assessorEditorOpen && !db.assessors.length && (
          <div className="empty assessor-empty">
            Nenhum contato cadastrado. Clique em <b>“Adicionar contato”</b> para incluir o primeiro.
          </div>
        )}
        {db.assessors.map((a) => (
          <div className="assessor-admin-row" key={a.id}>
            <div className="assessor-info">
              <b>{a.name}</b>
            ...
            <div className="assessor-actions">
              <button className="outline" onClick={() => editAssessor(a)}>
                Editar
              </button>
              <button className="danger" onClick={() => removeAssessor(a.id)}>
                Excluir
              </button>
            </div>
          </div>
    ...
    content = (
      <div className="panel">
        <h2>Contatos da assessoria</h2>
        <p>Contatos disponibilizados pela coordenação.</p>
        {!db.assessors.length ? (
          <div className="empty">Nenhum contato da assessoria foi cadastrado ainda.</div>
        ) : (
          db.assessors.map((a) => (
            <div className="contact-card" key={a.id}>
              <b>{a.name}</b>
              {a.role && <strong>{a.role}</strong>}
              {a.phone && <span>☎ {a.phone}</span>}
              {a.email && <span>✉ {a.email}</span>}
  ...
  else if (view === "admins" && admin)
    content = (
      <div className="panel">
        <div className="page-head">
          <div>
            <h2>Administradores</h2>
            <p>Qualquer administrador pode atualizar nome e e-mail. CPF e senha permanecem protegidos.</p>
          </div>
          <button className="primary" onClick={() => go("admin-release")}>
            ＋ Novo administrador
          </button>
        </div>
        {editingAdmin && (
          <div className="assessor-modal-backdrop" role="dialog" aria-modal="true">
            <div className="assessor-modal">
              <form onSubmit={saveAdminProfile}>
                ...
                <label className="field">
                  <span>CPF</span>
                  <input value={cpfBR(editingAdmin.cpf)} disabled />
                </label>
                <p className="auth-note">A senha não é exibida nem alterada nesta tela.</p>
                <div className="assessor-actions">
                  <button type="button" className="outline" onClick={cancelAdminEdit}>
                    Cancelar
                  </button>
        ...
        {db.admins.length ? (
          db.admins.map((a) => (
            <div className="assessor-admin-row" key={a.id}>
              <div className="assessor-info">
                <b>{a.name}</b>
                <small>
                  {cpfBR(a.cpf)} • {a.email || "Sem e-mail"}
                </small>
                <span>Cadastrado em {new Date(a.created).toLocaleDateString("pt-BR")}</span>
              </div>
              <div className="assessor-actions">
                <button className="outline" onClick={() => editAdmin(a)}>
                  Editar
                </button>
              </div>
            </div>
          ...
          <div className="empty">Nenhum administrador cadastrado.</div>
        )}
      </div>
    );
  ...
  if (view === "dashboard")
    content = (
      <>
        {content}
        <div className="panel activity-dashboard-callout">
          <div>
            <span className="activity-callout-icon">▣</span>
            <h2>{admin ? "Atividades diárias das lideranças" : "Registro de atividade diária"}</h2>
            <p>{admin ? "Visualize as imagens e descrições enviadas pelas lideranças e defira os registros analisados." : "Registre as atividades do dia com descrição, imagens em grupo, reuniões, visitas e abordagens."}</p>
          </div>
          <button className="primary" onClick={() => setView("daily-activities")}>
            {admin ? "Analisar registros" : "Registrar atividade"}
          </button>
        </div>
      </>
    );
    ...
    <div className="app-shell">
      ...
      <div className="main">
        <header className="topbar">
          <div>
            <h1>{title}</h1>
            <p>{admin ? "Visão geral dos cadastros e equipes." : "Gerencie somente os ativistas vinculados à sua liderança."}</p>
          </div>
          {admin && (
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              <button className="outline top-export" onClick={exportExcel}>
                ⇩ Exportar Excel
              </button>
              <button className="outline top-export" onClick={exportCSV}>
                ⇩ Exportar CSV
              </button>
            </div>
          )}
