...
const fresh = () => ({ leaderships: [], activists: [], admins: [], assessors: [] });
...
function Side({ admin, view, setView, logout, newL, newA, exportExcel, exportCSV }) {
  return <aside className="sidebar"><div className="brand"><b>Cadastro Eleitoral</b><span>{admin ? "Coordenadora Dainara Torres" : "Área da liderança"}</span></div><div className="nav-title">Menu</div><nav><button className={view === "dashboard" ? "active" : ""} onClick={() => setView("dashboard")}>⌂ Dashboard</button>{admin ? <><button className={view === "leaderships" ? "active" : ""} onClick={() => setView("leaderships")}>♙ Lideranças</button><button className={view === "activists" ? "active" : ""} onClick={() => setView("activists")}>♧ Ativistas</button><button className={view === "admins" ? "active" : ""} onClick={() => setView("admins")}>◉ Administradores</button><button className={view === "assessors" ? "active" : ""} onClick={() => setView("assessors")}>☎ Assessoria</button><button onClick={newL}>＋ Nova liderança</button></> : <><button className={view === "activists" ? "active" : ""} onClick={() => setView("activists")}>♧ Meus ativistas</button><button onClick={newA}>＋ Adicionar ativista</button><button className={view === "profile" ? "active" : ""} onClick={() => setView("profile")}>👤 Meu cadastro</button><button className={view === "assessors" ? "active" : ""} onClick={() => setView("assessors")}>☎ Contatos da assessoria</button></>}</nav>{admin && <><button className="export-side" onClick={exportExcel}>⇩ Exportar Excel</button><button className="export-side" onClick={exportCSV}>⇩ Exportar CSV</button></>}<button className="logout" onClick={logout}>Sair</button></aside>;
}
...
export default function Portal() {
  ...
  else if(view==="leadership-detail"&&admin){const l=db.leaderships.find(x=>x.id===detail),team=db.activists.filter(x=>x.leaderId===detail);content=<Detail title="Liderança" person={l} onBack={()=>setView("leaderships")} onEdit={()=>edit(l,"leader")} credential={<AdminLeadershipCredential person={l} onReset={() => { const password = makePassword(); setDb((d) => ({ ...d, leaderships: d.leaderships.map((item) => item.id === l.id ? { ...item, password, updated: Date.now() } : item) })); setMsg("Nova senha gerada. Copie-a e entregue à liderança."); return password; }} />} team={<div className="panel team-panel"><div className="page-head"><div><h3>Ativistas vinculados: {team.length}</h3><p>Equipe delimitada para facilitar a gestão.</p></div><button className="primary" onClick={()=>startA(l?.id)}>＋ Adicionar ativista</button></div>{team.length?team.map(a=><div className="list-item clickable" key={a.id} onClick={()=>{setDetail(a.id);setView("activist-detail")}}><div><b>{a.name}</b><small>{cpfBR(a.cpf)} • Título {a.title}</small></div><span>Ver cadastro →</span></div>):<div className="empty">Nenhum ativista vinculado.</div>}</div>}/>}
