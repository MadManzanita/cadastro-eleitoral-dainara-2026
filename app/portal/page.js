...
function TrustNetworkManager({ db, setDb, admin, remote }) {
  const [query, setQuery] = useState(""), [activistFilter, setActivistFilter] = useState(""), [leaderFilter, setLeaderFilter] = useState(""), [neighborhoodFilter, setNeighborhoodFilter] = useState(""), [originFilter, setOriginFilter] = useState(""), [editingFamily, setEditingFamily] = useState(null), [history, setHistory] = useState([]), [notice, setNotice] = useState("");
  ...
  const save = async (event) => {
    ...
    if (!editingFamily.name?.trim() || !editingFamily.address?.trim() || !editingFamily.municipality || !editingFamily.neighborhood) {
      setNotice("Informe nome, endereço, município e bairro/localidade."); return;
    }
    if (editingFamily.cpf && !cpfOK(editingFamily.cpf)) { setNotice("O CPF informado é inválido."); return; }
    try {
      const result = await remote("/api/families", { method: "POST", body: JSON.stringify({ action: "save", ...editingFamily }) });
      setDb((current) => ({ ...current, families: editingFamily.id ? current.families.map((x) => x.id === result.item.id ? result.item : x) : [result.item, ...current.families] }));
      setEditingFamily(null); setNotice(editingFamily.id ? "Cadastro atualizado." : "Pessoa cadastrada pela liderança.");
  ...
  const renderRow = (item) => {
    const activist = db.activists.find((x) => x.id === item.activistId);
    const leader = db.leaderships.find((x) => x.id === item.leaderId);
    return <div className="assessor-admin-row" key={item.id}><div className="assessor-info"><b>{item.name}</b><small>{item.cpf ? cpfBR(item.cpf) : "CPF não informado"} • {item.neighborhood||"Sem bairro"}</small><span>{item.activistId ? `Ativista responsável: ${activist?.name||"Não localizado"}` : `Cadastro direto da liderança: ${leader?.name||"Não localizada"}`}</span><span>Liderança responsável: {leader?.name||"Não localizada"}</span></div><div className="assessor-actions"><button className="outline" onClick={()=>{setHistory([]);setEditingFamily({...EMPTY,...item})}}>Editar</button><button className="outline" onClick={()=>showHistory(item)}>Histórico</button><button className="danger" onClick={()=>remove(item)}>Excluir</button></div></div>;
  };
  const groups = [
    { key: "leader", title: "Cadastros feitos pela liderança", description: "Pessoas incluídas diretamente pela liderança responsável.", items: list.filter((item) => !item.activistId) },
    { key: "activist", title: "Cadastros feitos pelos ativistas", description: "Pessoas incluídas pelos ativistas através do link da liderança.", items: list.filter((item) => item.activistId) }
  ];
  return <div className="panel"><div className="page-head"><div><h2>Rede de confiança</h2><p>{admin ? "Todas as redes cadastradas no sistema, organizadas pela origem do cadastro." : "Cadastros da própria liderança e dos ativistas vinculados, organizados separadamente."}</p></div>{!admin&&<button className="primary" onClick={()=>{setHistory([]);setNotice("");setEditingFamily({...EMPTY})}}>＋ Cadastrar pessoa</button>}</div>
    <input className="search" value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Buscar por nome ou CPF..."/>
    <div className="form-grid three"><label className="field"><span>Origem do cadastro</span><select value={originFilter} onChange={(e)=>setOriginFilter(e.target.value)}><option value="">Todas</option><option value="leader">Liderança</option><option value="activist">Ativista</option></select></label><label className="field"><span>Filtrar por ativista</span><select value={activistFilter} onChange={(e)=>setActivistFilter(e.target.value)}><option value="">Todos</option>{db.activists.map((a)=><option key={a.id} value={a.id}>{a.name}</option>)}</select></label>{admin&&<label className="field"><span>Filtrar por liderança</span><select value={leaderFilter} onChange={(e)=>setLeaderFilter(e.target.value)}><option value="">Todas</option>{db.leaderships.map((l)=><option key={l.id} value={l.id}>{l.name}</option>)}</select></label>}<label className="field"><span>Filtrar por bairro</span><select value={neighborhoodFilter} onChange={(e)=>setNeighborhoodFilter(e.target.value)}><option value="">Todos</option>{neighborhoods.map((n)=><option key={n} value={n}>{n}</option>)}</select></label></div>
    ...
    {editingFamily&&<form onSubmit={save} className="assessor-editor"><div className="editor-title"><div><h3>{editingFamily.id ? "Editar pessoa da rede" : "Cadastro da liderança na Rede de confiança"}</h3><p>Somente nome, endereço e localização territorial são obrigatórios.</p></div><button type="button" className="back small-back" onClick={()=>setEditingFamily(null)}>Cancelar</button></div>{editingFamily.id&&<div className="context-badge">Os vínculos com ativista e liderança serão preservados.</div>}<h3>Dados pessoais</h3><div className="form-grid"><Field f={editingFamily} setF={setEditingFamily} n="name" label="Nome completo" required/><Field f={editingFamily} setF={setEditingFamily} n="birth" label="Data de nascimento" type="date"/><Field f={editingFamily} setF={setEditingFamily} n="cpf" label="CPF" placeholder="000.000.000-00"/><Field f={editingFamily} setF={setEditingFamily} n="phone" label="Celular / telefone"/><Field f={editingFamily} setF={setEditingFamily} n="address" label="Endereço" required/><Field f={editingFamily} setF={setEditingFamily} n="mother" label="Nome da mãe"/><Field f={editingFamily} setF={setEditingFamily} n="email" label="E-mail" type="email"/><Field f={editingFamily} setF={setEditingFamily} n="cep" label="CEP"/></div><TerritoryFields f={editingFamily} setF={setEditingFamily}/><h3>Dados eleitorais</h3><div className="form-grid three"><Field f={editingFamily} setF={setEditingFamily} n="title" label="Título de eleitor"/><Field f={editingFamily} setF={setEditingFamily} n="zone" label="Zona"/><Field f={editingFamily} setF={setEditingFamily} n="section" label="Seção"/></div><button className="primary">{editingFamily.id ? "Salvar alterações" : "Concluir cadastro"}</button></form>}
    {list.length?groups.map((group)=><section className="trust-origin-group" key={group.key}><div className="trust-origin-head"><div><h3>{group.title}</h3><p>{group.description}</p></div><span>{group.items.length}</span></div>{group.items.length?group.items.map(renderRow):<div className="empty compact">Nenhum cadastro nesta categoria.</div>}</section>):<div className="empty">Nenhum cadastro encontrado.</div>}
  </div>;
}
