...
function activist(row, credential) {
  return {
    id: row.id,
    leaderId: row.leadership_id,
    name: row.name,
    birth: row.birth,
    cpf: row.cpf,
    phone: row.phone,
    ...
    pixname: row.name,
    bank: row.bank,
    trustCredentialed: Boolean(credential),
    trustCredentialedAt: credential?.created_at || null,
    created: row.created_at,
    updated: row.updated_at,
  };
}
...
export async function GET(request) {
  try {
    ...
    const leadershipQuery = session.role === "admin" ? db.from("leaderships").select("*").order("created_at", { ascending: false }) : db.from("leaderships").select("*").eq("id", session.id);
    const activistsQuery = session.role === "admin" ? db.from("activists").select("*").order("created_at", { ascending: false }) : db.from("activists").select("*").eq("leadership_id", session.id).order("created_at", { ascending: false });
    ...
    const credentialByActivist = new Map();
...
function fields(body) {
  return {
    ...
    cep: body.cep || null,
    title: onlyDigits(body.title).slice(0, 12) || null,
    electoral_zone: onlyDigits(body.zone).slice(0, 3) || null,
    electoral_section: onlyDigits(body.section).slice(0, 4) || null,
    pix: String(body.pix || "").trim() || null,
    pix_name: String(body.name || "").trim() || null,
    bank: body.bank || null,
  };
}
...
export async function POST(request) {
  try {
    ...
    if (body.action === "save-activist") {
      ...
      if (body.id) {
        const ownership = session.role === "admin" ? db.from("activists").select("id").eq("id", body.id) : db.from("activists").select("id").eq("id", body.id).eq("leadership_id", session.id);
        ...
        const { data, error } = await db
          .from("activists")
          .update({ ...values, leadership_id: leadershipId })
          .eq("id", owned.id)
          .select("*")
          .single();
        if (error) throw error;
    ...
    if (body.action === "save-assessor") {
      ...
      const query = body.id ? db.from("assessors").update(values).eq("id", body.id) : db.from("assessors").insert(values);
